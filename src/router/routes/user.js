...
module.exports = (app,db) => {
    ...
    app.get('/v1/admin/users/', (req,res) =>{
        ...
        let user_object;
        try {
            user_object = jwt.verify(req.headers.authorization.split(' ')[1],"SuperSecret");
        } catch (e) {
     ...
     */
     app.get('/v1/user/:id', (req,res) =>{
        db.user.findOne({include: 'beers',where: { id : req.params.id}})
            ...
            .catch((e)=>{ if(!res.headersSent){ res.status(500).json({error:e.toString()}); } });
});
    /**
...
app.post('/v1/user/token', async (req, res) => {
  try {
    const userEmail = String(req.body?.email ?? '').trim();
    ...
    if (!users || users.length === 0) {
      // keep your logic as-is if you want, but never crash
    ...
    if ((user0.password == userPassword) || (md5(user0.password) == userPassword)) {
      const jwtTokenSecret = "SuperSecret"; // keep as-is if you haven't fixed secrets yet
      const payload = { id: user0.id, role: user0.role };

      const token = jwt.sign(payload, jwtTokenSecret, { expiresIn: 86400 });
      return res.status(200).json({ jwt: token, user: users });
    }
